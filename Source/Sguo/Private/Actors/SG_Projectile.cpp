 ...
 * 
 * 详细流程：
 * 1. 检查是否是无效碰撞（自身、Owner、Instigator）
 * 2. 检查是否是单位，如果是同阵营则忽略
 * 3. 检查是否是主城，如果是同阵营则忽略
 * 4. 处理敌方目标碰撞
 * 5. 处理地面/墙壁碰撞
...
void ASG_Projectile::HandleProjectileImpact(AActor* OtherActor, const FHitResult& Hit)
{
	...
	if (TargetUnit)
	{
		...
		{
			UE_LOG(LogSGGameplay, Verbose, TEXT("  碰撞友方单位，忽略穿过：%s"), *TargetUnit->GetName());
			return;
		}

		// 检查是否已击中（穿透模式下可能已经击中过）
		...
		{
			UE_LOG(LogSGGameplay, Verbose, TEXT("  目标已死亡，忽略：%s"), *TargetUnit->GetName());
			return;
		}
	...
	if (TargetMainCity)
	{
		...
		{
			UE_LOG(LogSGGameplay, Verbose, TEXT("  碰撞友方主城，忽略穿过：%s"), *TargetMainCity->GetName());
			return;
		}
		...
		{
			UE_LOG(LogSGGameplay, Verbose, TEXT("  主城已被摧毁，销毁投射物"));
			Destroy();
			return;
		}
	...
	// ========== 处理地面/墙壁碰撞 ==========
	// 不是单位也不是主城，视为地形碰撞
	UE_LOG(LogSGGameplay, Verbose, TEXT("  碰撞地形/墙壁：%s"), *OtherActor->GetName());

	// 构建击中信息
	FSGProjectileHitInfo HitInfo;
	HitInfo.HitActor = OtherActor;
	HitInfo.HitLocation = Hit.ImpactPoint.IsNearlyZero() ? GetActorLocation() : FVector(Hit.ImpactPoint);
	HitInfo.HitNormal = Hit.ImpactNormal.IsNearlyZero() ? FVector::UpVector : FVector(Hit.ImpactNormal);
	HitInfo.ProjectileDirection = CurrentVelocity.GetSafeNormal();
	HitInfo.ProjectileSpeed = CurrentVelocity.Size();
